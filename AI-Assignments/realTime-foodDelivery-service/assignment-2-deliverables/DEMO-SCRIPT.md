...
> "Operations see failure counts but not root causes. Our sample program: **import multi-domain CSV → join on order_id → rule-based correlation → template narrative → recommendations**. All queries go through the **gateway on port 8080**, which proxies to `analytics-service`."
...
## Option A — REST via gateway (recommended for video)
...
- Show `third-assignment-sample-data-set/` (8 CSV files).
...
- Deliverables: `ASSIGNMENT-2-WRITEUP.docx`, `SAMPLE-USE-CASE-OUTPUTS.docx`, GitHub repo.
- Optional: analytics dashboard at http://localhost:3002 (same APIs behind gateway).
...
3. **Say:** "The UI calls the same gateway APIs — the sample program is the backend analytics module."
...
- [ ] `./scripts/refresh_analytics_outputs.py` — outputs current  
- [ ] Microphone tested  
