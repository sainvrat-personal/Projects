...
**Folder contents:**
1. `ASSIGNMENT-2-WRITEUP.docx` — problem solution write-up with architecture overview
2. Demo video recording (8–10 min) — local walkthrough of all six sample use cases
3. `SAMPLE-USE-CASE-OUTPUTS.md` — recorded outputs for UC1–UC6 (with JSON evidence in `raw-responses/`)

**How to run locally:**
```bash
git clone [YOUR_GITHUB_REPO_URL]
cd realTime-foodDelivery-service
cp .env.example .env
