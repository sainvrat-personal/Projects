#!/usr/bin/env python3
"""Generate assignment-2-deliverables/ASSIGNMENT-2-WRITEUP.docx from project content."""
...
OUT = ROOT / "assignment-2-deliverables" / "ASSIGNMENT-2-WRITEUP.docx"
...
def main():
    ...
    add_bullets(
        doc,
        [
            "REST query APIs (via analytics-service, proxied through backend-service :8080).",
    ...
    diagram.add_run(
        "CSV files (orders, clients, warehouses, drivers, warehouse_logs, fleet_logs, "
        "feedback, external_factors)\n"
        "    → SampleDataImportService (batch load into PostgreSQL analytics.* schema)\n"
        "    → AnalyticsOrderRepository (SQL joins / aggregations)\n"
        "    → CorrelationEngine (rule-based multi-domain matching)\n"
    ...
    add_para(doc, "Sample dataset: third-assignment-sample-data-set/", bold=True)
    ...
    add_para(
        ...
        "assignment-2-deliverables/SAMPLE-USE-CASE-OUTPUTS.md"
    )
    ...
    add_bullets(
        ...
        [
            ...
            "See README.md and assignment-2-deliverables/DEMO-SCRIPT.md for video recording steps",
        ],
    )
